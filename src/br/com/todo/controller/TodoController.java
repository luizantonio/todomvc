...
public class TodoController { // implements java.io.Serializable {
...
//		}
	...
	public String editTodo(Todo todo) {	
	    ...
	    String id_h = ec.getRequestParameterMap().get("id_editar_hidden");
		...
		if(todo != null) {
			HttpSession session = (HttpSession) FacesContext
					...
					.getSession(true);
			session.setAttribute("title",todo.getTitle());
	...
	public String updateTodo() {
		...
		if(this.getTitle() != null && this.getTitle().trim().length() > 0) {
			 ...
			 */
			try {
				result = this.service.update(this.todo.getId(), this.getTitle());
			} catch (Exception e) {				
				e.printStackTrace();
			}
		}
		...
		//FacesContext.getCurrentInstance().renderResponse();
		this.setRETURN_VIEW("index.xhtml");		
		return this.RETURN_VIEW;
	}
